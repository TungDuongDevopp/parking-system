...
using ParkingSystem.Sessions;
using ParkingSystem.Web.Models.Account;
using ParkingSystem.Web.Views.Shared.Components.TenantChange;
...
public class AccountController : ParkingSystemControllerBase
{
    ...
    public virtual async Task<JsonResult> Login(LoginViewModel loginModel, string returnUrl = "", string returnUrlHash = "")
    {
        ...
        try
        {
            ...
            if (returnUrl == Url.Action("Index", "Users"))
            {
                var isGranted = await _userManager.IsGrantedAsync(loginResult.User.Id, PermissionNames.Pages_Users);
                if (!isGranted)
                {
        ...
        {
            // Return a JSON error response so the UI can show a friendly message instead of the exception propagating
            return Json(new AjaxResponse(new UserFriendlyException(ex.Message)));
        }
    }
